...
まだ Next.js プロジェクトは未生成。このリポジトリには設計資料と移植元プロトタイプ
...
問題形式は3種: `zenshi`(全肢判定・実装済み)/ `calc`(途中式ビルダー・未実装)/ `spot`(間違い探し・未実装)。
`type` で分岐するため、calc / spot を後から足してもデータ構造は壊れない。
...
- **移植の忠実性**: 画面・挙動・得点はプロトタイプと一致させる(6テーマ・満点34点)
...
| v1 | Next.js化、localStorage、業法+民法の拡充 | localStorage |
